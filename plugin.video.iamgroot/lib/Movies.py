...
#    try:
    ...
    for channel,block in channel_block:
        prog = re.compile('<a qt-title="(.+?)".+?>(.+?)</div>(.+?)<br>',re.DOTALL).findall(str(block))
        for time_info,genre,desc in prog:
            time,name,year = re.findall('<(.+?) (.+?)\((.+?)\)','<'+str(time_info))[0]
            time_finder = re.compile('(.+?)-(.+?)>').findall(str(time+'>'))
