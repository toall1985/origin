...
def League_Tables(url):
    addDirFolder('Premier League','http://www.sportinglife.com/football/premier-league/table',6,ICON,FANART,'')
    addDirFolder('Scottish Premier','http://www.sportinglife.com/football/scottish-premier/table',6,ICON,FANART,'')
    addDirFolder('Championship','http://www.sportinglife.com/football/championship/table',6,ICON,FANART,'')
    addDirFolder('Champions League','http://www.sportinglife.com/football/champions-league/table',12,ICON,FANART,'')
    addDirFolder('Europa League','http://www.sportinglife.com/football/europa-league/table',12,ICON,FANART,'')
    addDirFolder('League One','http://www.sportinglife.com/football/league-one/table',11,ICON,FANART,'')
    addDirFolder('League Two','http://www.sportinglife.com/football/league-two/table',11,ICON,FANART,'')
    addDirFolder('Scottish Championship','http://www.sportinglife.com/football/scottish-championship/table',11,ICON,FANART,'')
    addDirFolder('Scottish League One','http://www.sportinglife.com/football/scottish-league-one/table',11,ICON,FANART,'')
    addDirFolder('Scottish League Two','http://www.sportinglife.com/football/scottish-league-two/table',11,ICON,FANART,'')
    addDirFolder('National League','http://www.sportinglife.com/football/national-league/table',11,ICON,FANART,'')
    addDirFolder('La Liga','http://www.sportinglife.com/football/la-liga/table',11,ICON,FANART,'')
    addDirFolder('Serie A','http://www.sportinglife.com/football/serie-a/table',11,ICON,FANART,'')
    addDirFolder('Bundesliga','http://www.sportinglife.com/football/bundesliga/table',11,ICON,FANART,'')
    addDirFolder('Ligue 1','http://www.sportinglife.com/football/ligue-1/table',11,ICON,FANART,'')
    addDirFolder('Eredivisie','http://www.sportinglife.com/football/eredivisie/table',11,ICON,FANART,'')
    addDirFolder('Portuguese Liga','http://www.sportinglife.com/football/portuguese-liga/table',11,ICON,FANART,'')

def champ_league(url):
    HTML = OPEN_URL(url)
    block = re.compile('class="hdr t2">(.+?)<span class="aside">(.+?)<h2 ',re.DOTALL).findall(HTML)
    for league,block in block:
        addDirFolder('[COLORwhite]                                                               '+(league).replace(' - Short table','')+'[/COLOR]','',11,ICON,FANART,'')
...
def Football_Highlights():
    ...
    addDirFolder('Footy Tube','http://www.footytube.com/leagues',13,ICON,FANART,'')
...
def footytube(url):
    ...
    for img,name in block_and_name:
        addDirFolder(name,'',14,footy+img,FANART,'')
...
def footytube_leagues(name):
    ...
    for img,block in block_and_name:
        ...
        for url,name,qty in leagues:
            addDirFolder(name + ' - ' + qty,footy+url,15,ICON,FANART,'')
        else:
            pass		
...
def footytube_teams(url):
    ...
    for url,name in match:
        addDirFolder(name,footy+url,16,ICON,FANART,'')
...
def footytube_videos(url):
    ...
    for url,img,name,age in match:
        addDir(name + ' - ' + age, footy+url,17,img,FANART,'')
...
def get_All_Rows(url,iconimage):
    ...
    for url,name in Next:
        addDirFolder('NEXT PAGE',url,1,iconimage,FANART,'')
    if len(match2)<=0:
        addDirFolder('No Replays available sorry',url,1,iconimage,FANART,'')
...
elif mode == 9 		: League_Tables(url)
elif mode == 10 	: Search()
elif mode == 11 	: Prem_Table2(url)
elif mode == 12 	: champ_league(url)
elif mode == 13 	: footytube(url)
elif mode == 14 	: footytube_leagues(name)
