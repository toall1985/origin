...
watched_list = xbmc.translatePath('special://home/userdata/addon_data/plugin.video.adultlife/watched')
...
def Menu(name, url, mode, iconimage, fanart, description, extra, showcontext=True, allinfo={}):
    ...
    if showcontext:
        contextMenu = []
        if showcontext == 'fav':
            contextMenu.append(('Remove from Adult Life Favorites', 'XBMC.RunPlugin(%s?mode=12&name=%s)'
                                % (sys.argv[0], urllib.quote_plus(name))))
            contextMenu.append(('Check for episode', 'XBMC.RunPlugin(%s?mode=41&name=%s)'
                                % (sys.argv[0], urllib.quote_plus(name))))								
        if not name in FAV:
            contextMenu.append(('Add to Adult Life Favorites',
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.adultlife-%s]: %s" % (addon_version, string))
...
def getFavourites():
    if not os.path.exists(favourites):
        ...
        favList.append(('Adult Life Favourites Section', '', '', '', '', '', ''))
        a = open(favourites, "w")
        a.write(json.dumps(favList))
        a.close()
    else:
...
def check_for_episode():
	...
	if not os.path.exists(watched_list):
		favList = []
		addon_log('Making Favorites File')
		favList.append(('Adult Life Watched Section', '', '', '', '', '', ''))
		a = open(watched_list, "w")
		a.write(json.dumps(favList))
		a.close()
	else:
