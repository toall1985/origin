...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('JokersTV Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    List=[]
    List.append(_EditJoker.MainBase)
    PassedUrls = 0
    FoundChannel = 1 
    ReadChannel = 0
    FoundMatch = 0
...
def urlsolver(url):
    ...
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(Jokers TV ,Using Urlresolver module.. - ,5000)")
        import urlresolver
