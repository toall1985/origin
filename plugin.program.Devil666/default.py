...
def Categories():
	...
	if mainmenu == 'true':
		extras.addDir('folder','Devils Origin Builds','none', 'buildmenu', 'Build_Menu.png','','','')
	if adultbuilds == 'true':
...
def Wipe_Kodi():
    ...
    if choice == 1:
        if skin!= "skin.confluence":
            dialog.ok('[COLOR=white]Devil 666[/COLOR]','Please switch to the default Confluence skin','before performing a wipe.','')
            ...
            choice = xbmcgui.Dialog().yesno("Remove Devil 666?", 'Do you also want to remove the Devil 666', 'add-on and have a complete fresh start or would you', 'prefer to keep this on your system?', yeslabel='Remove',nolabel='Keep')
            if choice == 0:
                cache.Remove_Textures()
                ...
                dp.create("[[COLOR=white]Devil 666[/COLOR][/B]","Checking ",'', 'Please Wait')
...
#Builds Section
def BuildMenu():
    link = OPEN_URL('https://archive.org/download/Devil666Wizard/Devil666-Wizard.txt').replace('\n','').replace('\r','')
...
def wizard(name,url,description):
    ...
    dp = xbmcgui.DialogProgress()
    dp.create("Devil 666 ","Downloading ",'', 'Please Wait')
