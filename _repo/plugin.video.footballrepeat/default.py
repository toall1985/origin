...
def get_All_Rows(url):
    HTML = OPEN_URL(url)
    match = re.compile('itemprop="image" class="entry-thumb" src="(.+?)".+?<h3 itemprop="name" class="entry-title td-module-title"><a itemprop="url" href="(.+?)" rel="bookmark" title=".+?">(.+?)</a></h3>',re.DOTALL).findall(HTML)
    for img,url,name in match:
        Name = name.replace('&#8211;', '-').replace('&#038;', '&')
        addDir('[COLORgreen]'+Name+'[/COLOR]',url,2,img,'','')
