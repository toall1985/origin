...
def Home_Menu():
    addDirFolder('Audio Books','',11,ART + 'icon.png',ART + 'fanart.jpg','')
    addDirFolder('Kids Audio Books','',6,ART + 'icon.png',ART + 'fanart.jpg','')

def Audio_Menu():	
    addDirFolder('All','',1,ART + 'icon.png',ART + 'fanart.jpg','')
    addDirFolder('Popular','',12,ART + 'icon.png',ART + 'fanart.jpg','')
    addDirFolder('Search','',13,ART + 'icon.png',ART + 'fanart.jpg','')

def Popular():
    HTML = OPEN_URL(BASE + 'books' + CAT)
    match = re.compile('<NAME=>(.*?)</NAME><URL=>(.*?)</URL><CAT=>(.*?)</CAT>').findall(HTML)
    for name,url,cat in match:
        if 'Parent' in name:
            pass
        elif '2' in cat:
            addDirFolder((name).replace('%20',' ').replace('.mp3',''),url,10,ART + 'icon.png',ART + 'fanart.jpg','')
    ...
    for name,url,cat in match:
        if Search_Name in name.lower():
            if '1' in cat:
                ...
                addDirFolder((name).replace('%20',' ').replace('.mp3',''),url,9,ART + 'icon.png',ART + 'fanart.jpg','')
...
def Match(url):
    ...
    for url,name in match2:
        if 'mp3' in name:
            addDir((name).replace('%20',' '),url2+url,2,ART + 'icon.png',ART + 'fanart.jpg','')
        if 'wma' in name:
            addDir((name).replace('%20',' '),url2+url,2,ART + 'icon.png',ART + 'fanart.jpg','')
        if 'm4b' in name:
            addDir((name).replace('%20',' '),url2+url,2,ART + 'icon.png',ART + 'fanart.jpg','')
        elif '/' in name:
            addDirFolder((name).replace('%20',' ').replace('/','').replace('.mp3',''),url2+url,9,ART + 'icon.png',ART + 'fanart.jpg','')
...
def Match2(url):
    ...
    for url,name in match:
        if 'Parent' in name:
            ...
            addDir((name).replace('%20',' ').replace('/','').replace('.mp3',''),url2+url,2,ART + 'icon.png',ART + 'fanart.jpg','')
        elif 'm4a' in name:
            addDir((name).replace('%20',' ').replace('/','').replace('.mp3',''),url2+url,2,ART + 'icon.png',ART + 'fanart.jpg','')
        else:
            addDirFolder((name).replace('%20',' ').replace('/','').replace('.mp3',''),url2+url,10,ART + 'icon.png',ART + 'fanart.jpg','')
...
elif mode == 9 		: Match(url)
elif mode == 10 	: Match2(url)
elif mode == 11 	: Audio_Menu()
elif mode == 12 	: Popular()
elif mode == 13 	: Search_Audio()
